    ...
    <nav role="navigation" aria-label="Pagination Navigation" class="flex justify-center mt-8">
        <ul class="inline-flex -space-x-px">
            ...
            @if ($books->onFirstPage())
                <li>
                    <span class="px-4 py-2 text-sm text-gray-400 bg-gray-700 rounded-l-lg cursor-not-allowed">← Previous</span>
                </li>
            @else
                <li>
            ...
            @for ($page = 1; $page <= min(3, $last); $page++)
                <li>
                    @if ($page == $current)
                        <span class="px-4 py-2 text-sm text-white bg-indigo-800 font-semibold">{{ $page }}</span>
                    @else
            ...
            @elseif ($last == 5)
                <li>
                    @if (4 == $current)
                        <span class="px-4 py-2 text-sm text-white bg-indigo-800 font-semibold">4</span>
                    @else
                        <a href="{{ $books->url(4) }}"
            ...
            @else
                <li>
                    <span class="px-4 py-2 text-sm text-gray-400 bg-gray-700 rounded-r-lg cursor-not-allowed">Next →</span>
                </li>
            @endif
        </ul>
    </nav>
